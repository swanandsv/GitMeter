...
api_key = "sk-ignQtYPHWsJh9U2nzPRbro1Z-Y0aATj08bdIByB74nT3BlbkFJUc8zwHWduPE03gGElMl4VQnvOOw-_Q95-SdQuit_QA"
...
def analyze_java_code_with_pmd(java_code):
    ...
    pmd_path = r'C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\bin\pmd.bat'
    
    # Multiple rulesets
    ruleset_paths = r'C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\design.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\bestpractices.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\codestyle.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\documentation.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\errorprone.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\multithreading.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\performance.xml,C:\Users\Swanand\Desktop\ASE_Project\ase_project\pmd-bin-7.4.0\extracted\category\java\security.xml'
...
def home(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            USERKEY = 'YWWDMZS7'
            VERSION = '8.0.0'
            subUserId = 'Swanand Vaishampayan'
...
def check_non_code_quality_status(request):
    username = request.GET.get('username', '')
    if username in non_code_quality_data:
        data = non_code_quality_data[username]
    else:
        data = {'status': 'pending'}
    return JsonResponse(data)
...
def clear_previous_data(request):
    ...
    matching_codes_count_2 = 0
    matching_codes_count = 0
    request.session.pop('technologies', None)  # Clear technologies from session
    #logging.debug("Previous data cleared")
        ...
        if form.is_valid():
            ...
            if github_link:
                ...
                if username:
                    ...
                    # Log that we are starting the background calculation
                    #logging.debug(f"Redirecting to non_code_quality_info for username: {username}")
                    
                    # Redirect to non_code_quality_info view
                    return redirect('non_code_quality_info', username=username)
                else:
                    return redirect('no_github')
            else:
...
def non_code_quality_info(request, username):
    technologies_from_session = request.session.get('technologies', [])
    # Start a background thread to calculate non-code quality metrics
    threading.Thread(target=calculate_non_code_quality_metrics, args=(username, technologies_from_session), daemon=True).start()
    
    # Render the informational page immediately
    return render(request, 'non_code_quality_info.html', {'username': username})

def results(request, username):
    # Clear old code quality data if a new resume is submitted
    if request.method == 'POST':
        code_quality_data.pop(username, None)
    
    # Check if non-code quality metrics are ready
    non_code_quality_metrics = non_code_quality_data.get(username, {'status': 'pending'})
    
    if non_code_quality_metrics['status'] == 'pending':
        # Render a page with a message that non-code quality metrics are being processed
        return render(request, 'non_code_quality_info.html', {'username': username})
    
    # Retrieve technologies from the session
    technologies = request.session.get('technologies', [])
    metrics = non_code_quality_metrics.get('metrics', {})
    ...
    #logging.debug(f"Starting background thread for code quality calculation for username: {username}")
    
    # Start code quality calculation in the background
    threading.Thread(target=run_code_quality_cal, args=(username,), daemon=True).start()
    
    # Initial status of code quality metrics
    data = code_quality_data.get(username, {'status': 'pending', 'metrics': None})
...
def code_quality_results(request):
    ...
    # Optionally add AJAX polling to check for readiness
    if not data['metrics']:
        return render(request, 'non_code_quality_info.html', {'username': username})
...
def list_languages(username):
    ...
    if repos_response.status_code != 200:
        #print(f"Error fetching repositories for user {username}. Status Code: {repos_response.status_code}")
        return None
    ...
    for repo in repos:
        languages_url = f"{base_url}/repos/{username}/{repo['name']}/languages"
        lang_response = requests.get(languages_url, headers=headers)
...
def identify_project_type(username, repo_name):
    ...
    if topics_response.status_code != 200:
        #print(f"Error fetching topics for repository {repo_name}. Status Code: {topics_response.status_code}")
        return "Unknown"
    
    topics = topics_response.json().get('names', [])

    # Define topic sets for different project types
    web_related_topics = {"web", "web-development", "django", "flask", "react", "vue", "angular", "express", "spring-boot", "html", "css", "javascript", "typescript"}
    ml_related_topics = {"machine-learning", "deep-learning", "tensorflow", "pytorch", "nlp", "data-science", "keras", "scikit-learn"}
    devops_related_topics = {"devops", "docker", "kubernetes", "ci-cd", "terraform", "ansible", "cloud", "jenkins"}

    # Check topics for project type indicators
    has_web_topic = any(topic in web_related_topics for topic in topics)
    has_ml_topic = any(topic in ml_related_topics for topic in topics)
    has_devops_topic = any(topic in devops_related_topics for topic in topics)

    # Determine project type based on topics
    if has_web_topic and has_ml_topic:
        return "Web-Based with ML"
    elif has_web_topic:
        return "Web-Based"
    elif has_ml_topic:
        return "Machine Learning"
    elif has_devops_topic:
        return "DevOps"

    # If topics do not indicate project type, check files in the repository
    contents_url = f"{base_url}/repos/{username}/{repo_name}/contents/"
    contents_response = requests.get(contents_url, headers=headers)
    ...
    for content in contents:
        ...
        # Check for common web-based project files
        if name_lower in [
            "package.json", "dockerfile", "nginx.conf", "index.html", "server.js", "app.py",
            "application.properties", "application.yml", "pom.xml", "build.gradle", "web.xml", "applicationcontext.xml",
            "manage.py", "settings.py", "urls.py", "wsgi.py", "gunicorn.conf.py", "supervisord.conf", "pytest.ini", "tox.ini"
        ]:
            has_web_files = True
        
        # Check for common machine learning project files
        elif name_lower.endswith(('.py', '.ipynb')) or name_lower in ["train.py", "predict.py", "test.py", "model.py"]:
            # Download the file content to analyze for ML frameworks
            file_content_response = requests.get(content['download_url'])
            if file_content_response.status_code == 200:
                file_content = file_content_response.text
                if any(keyword in file_content for keyword in ["tensorflow", "keras", "scikit-learn", "pytorch", "training", "testing"]):
        ...
        # Check for common DevOps-related files
        elif name_lower in ["dockerfile", "jenkinsfile", "terraform.tf", "ansible.cfg", "kubernetes.yaml", "helm.yaml", "cloudformation.yaml", "vagrantfile"]:
            has_devops_files = True
    
    # Determine project type based on file content if topics were not conclusive
    if has_web_files and has_ml_files:
        return "Web-Based with ML"
    elif has_web_files:
        return "Web-Based"
    elif has_ml_files:
        return "Machine Learning"
    elif has_devops_files:
        return "DevOps"
    else:
        return "Unknown"
...
def extract_comments(code):
    # For Python, JavaScript, and Java, look for line comments and block comments
    comments = []
    lines = code.split('\n')
    for line in lines:
        if line.strip().startswith('#') or line.strip().startswith('//') or '/*' in line or '*/' in line:
...
def fetch_repo_details(username, repo_name):
    ...
    if repo_response.status_code != 200:
        #print(f"Error fetching repository details for {repo_name}. Status Code: {repo_response.status_code}")
        return None
    
    repo_data = repo_response.json()
    
    # Fetch repository forks, watchers, stars
    forks_count = repo_data.get('forks_count', 0)
    watchers_count = repo_data.get('watchers_count', 0)
    stars_count = repo_data.get('stargazers_count', 0)

    # Fetch repository badges (if available in README)
    readme_url = f"{base_url}/repos/{username}/{repo_name}/readme"
    readme_response = requests.get(readme_url, headers=headers)
    ...
    if readme_response.status_code == 200:
        readme_data = readme_response.json()
        readme_content = requests.get(readme_data['download_url']).text
        # Extract badges from README content (basic example)
        badges = [line for line in readme_content.split('\n') if '![badge]' in line]
...
def fetch_user_pull_requests(username):
    ...
    if pull_requests_response.status_code != 200:
        #print(f"Error fetching pull requests for user {username}. Status Code: {pull_requests_response.status_code}")
        return []
    
    pull_requests_data = pull_requests_response.json()
    return pull_requests_data.get('items', [])

def _gather_non_code_quality_metrics(username, technologies_from_session,parsed_skills):
    ...
    structured_repos_count = 0
    
    for repo in repos:
        repo_name = repo['name']
        if repo['fork']:
                total_forked_repos += 1  # Increment fork count
        structure = check_project_structure(username, repo_name)
        project_type = identify_project_type(username, repo_name)
        documentation = check_documentation(username, repo_name)
        ...
        # Check if repo has both a README and tests
        if structure and 'has_tests' in structure:
            if structure['has_tests']:
                structured_repos_count += 1
        
        # Add further analysis as needed
        forks_count, watchers_count, stars_count, badges = fetch_repo_details(username, repo_name)
        
        # Add repository details to the analysis results
    ...
    pull_requests = fetch_user_pull_requests(username)
    total_repos = len(repos)
    undocumented_repos_count = total_repos - documented_repos_count
    unstructured_repos_count = total_repos - structured_repos_count
    
    # Use the passed technologies list
    extra_technologies = set(languages.keys()) - set(technologies_from_session)
    ...
    return {
        ...
        "total_repos": total_repos,
        "documented_repos": documented_repos_count,
        "undocumented_repos": undocumented_repos_count,
        "structured_repos": structured_repos_count,
        "unstructured_repos": unstructured_repos_count,
        "aggregate_metrics": {
            "forks": sum(result['forks'] for result in analysis_results),
            "watchers": sum(result['watchers'] for result in analysis_results),
            "stars": sum(result['stars'] for result in analysis_results),
